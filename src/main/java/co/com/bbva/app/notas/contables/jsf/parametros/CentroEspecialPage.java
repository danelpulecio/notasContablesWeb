...
public class CentroEspecialPage extends GeneralParametrosPage<CentroEspecial, CentroEspecial> {
	...
	public Collection<CentroEspecial> _buscarPorFiltro() throws Exception {
		if(!param.isEmpty()){
			LOGGER.info("{} Buscar centro especial: {}", session.getTraceLog(),param );
		}
	...
	protected boolean _guardar() throws Exception {
		...
		try {
			if (objActual.getCodigo().intValue() <= 0) {
				LOGGER.info("{} Crea centro especial: {}", session.getTraceLog(),  objActual.getNombreSucursal()+ " "+ objActual.getCodigoSucursal() );
				notasContablesManager.addCentroEspecial(objActual, getCodUsuarioLogueado());
			} else {
				LOGGER.info("{} Actualiza centro especial: {}", session.getTraceLog(),  objActual.getCodigo() +" "+  objActual.getNombreSucursal()+ " "+ objActual.getCodigoSucursal() );
				notasContablesManager.updateCentroEspecial(objActual, getCodUsuarioLogueado());
			}
		} catch (Exception e) {
			objActual.setCodigo(codInicial);
			LOGGER.error("{} Ya existe un centro especial con la misma sucursal: {}", session.getTraceLog(),  codInicial , e );
	...
	public boolean _cambiarEstado() throws Exception {
		LOGGER.info("{} Cambia el estado centro especial: {}", session.getTraceLog(),  notasContablesManager.getCentroEspecial(objActual).getCodigo() +" "+  notasContablesManager.getCentroEspecial(objActual).getCodigoSucursal()+ " "+ notasContablesManager.getCentroEspecial(objActual).getEstado() );
	...
	public boolean _borrar() throws Exception {
		try {
			LOGGER.info("{} Borra Centro Especial: {}", session.getTraceLog(),  objActual.getCodigo() +" "+  objActual.getCodigoSucursal() );
			notasContablesManager.deleteCentroEspecial(objActual, getCodUsuarioLogueado());
		} catch (Exception e) {
			LOGGER.error("{} No puede eliminar el Centro Especial porque contiene registros asociados: {}", session.getTraceLog(),  objActual.getCodigo() ,e );
