...
public class InstanciaDAO extends CommonSeqDAO<Instancia> {
    ...
    protected Object[] getDataToAdd(Connection con, Instancia row) throws Exception {
        ...
        if (row.getFechaHoraInicioTs() != null) {
        }
        return new Object[]{row.getCodigo().intValue(), row.getFechaHoraInicioTs(), row.getCodigoNotaContable().intValue(), row.getCodigoSucursalOrigen(), row.getCodigoUsuarioActual().intValue(), row.getUltimaActualizacionTs(), row.getEstado()};
    }
