...
public class ImpuestoPage extends GeneralParametrosPage<Impuesto, Impuesto> {
	...
	public Collection<Impuesto> _buscarPorFiltro() throws Exception {
		if(!param.isEmpty()){
			LOGGER.info("{} Buscar impuesto: {}", session.getTraceLog(),param );
		}
	...
	protected boolean _guardar() throws Exception {
		...
		try {
			if (objActual.getCodigo().intValue() <= 0) {
				LOGGER.info("{} Crea impuesto: {}", session.getTraceLog(),objActual.getNombre()+" "+objActual.getValor() );
				notasContablesManager.addImpuesto(objActual, getCodUsuarioLogueado());
			} else {
				LOGGER.info("{} Actualiza impuesto: {}", session.getTraceLog(), objActual.getCodigo()+" "+objActual.getNombre()+" "+objActual.getValor() );
	...
	public boolean _cambiarEstado() throws Exception {
		LOGGER.info("{} Cambia estado impuesto: {}", session.getTraceLog(), notasContablesManager.getImpuesto(objActual).getCodigo()+" "+notasContablesManager.getImpuesto(objActual).getNombre()+" "+notasContablesManager.getImpuesto(objActual).getEstado() );
	...
	public boolean _borrar() throws Exception {
		LOGGER.info("{} Borrar estado impuesto: {}", session.getTraceLog(), objActual.getCodigo()+" "+objActual.getNombre() );
