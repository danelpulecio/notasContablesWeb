...
public class EnteAutorizadorPage extends GeneralParametrosPage<EnteAutorizador, EnteAutorizador> {
	...
	public Collection<EnteAutorizador> _buscarPorFiltro() throws Exception {
		if(!param.isEmpty()){
			LOGGER.info("{} Buscar ente autorizado: {}", session.getTraceLog(),param );
		}
	...
	protected boolean _guardar() throws Exception {
		...
		try {
			...
			LOGGER.info("{} Crea ente autorizador: {}", session.getTraceLog(),objActual.getCodigo() + " "+ objActual.getNombreUsuario() +" "+ objActual.getCodigoSucursal() );
	...
	public boolean _cambiarEstado() throws Exception {
		LOGGER.info("{} Cambiar estado ente autorizador: {}", session.getTraceLog(),notasContablesManager.getEnteAutorizador(objActual).getCodigo() + " "+ notasContablesManager.getEnteAutorizador(objActual).getNombreUsuario() +" "+ notasContablesManager.getEnteAutorizador(objActual).getCodigoSucursal() +" "+ notasContablesManager.getEnteAutorizador(objActual).getEstado() );
	...
	public boolean _borrar() throws Exception {
		try {
			LOGGER.info("{} Borra ente autorizador: {}", session.getTraceLog(),objActual.getCodigo() + " "+ objActual.getNombreUsuario() +" "+ objActual.getCodigoSucursal()  );
			notasContablesManager.deleteEnteAutorizador(objActual, getCodUsuarioLogueado());
		} catch (Exception e) {

			LOGGER.error("{} No puede eliminar el Ente Autorizador porque contiene registros asociados: {}", session.getTraceLog(),objActual.getCodigo() , e );
	...
	private void consultarListasAuxiliares() {
		if (esUltimaFase()) {
			try {
				...
				LOGGER.error("{} Error al inicializar el mdulo de administracin de entes autorizados", session.getTraceLog() ,e );
